...
export const ModalEditarCliente = () => {
  ...
  const idPago = useUIStore((state) => state.idPago);
  const closeModalEditarCliente = useUIStore((state) => state.closeModalEditarCliente);
  const updatePagoCliente = useUIStore((state) => state.updatePagoCliente);
  ...
  const getPagocliente = async (idPago) => {
    try {
      const resp = await fetch(`/api/pago-especifico?id=${idPago}`);
      const tourData = await resp.json();
      setInfoCliente(tourData);
    } catch (error) {
